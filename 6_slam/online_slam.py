...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # n is the index of the robot pose in the matrix/vector
        n =  2 
        # print('omega size, ', Omega.dimx, Omega.dimy)
        # print('Xi size, ', Xi.dimx, Xi.dimy, Xi)
    
        measurement = data[k][0]
        motion      = data[k][1]
        
        expand_inds = [0,1]
        expand_inds.extend(list(range(4,Omega.dimx+2)))
        Omega = Omega.expand(Omega.dimx+2, Omega.dimy+2, expand_inds, expand_inds)
        # print(Omega)
        Xi = Xi.expand(Xi.dimx+2, Xi.dimy, expand_inds, [0])
        # print(Xi)
        ...
        A = Omega.take([0, 1], list(range(2,Omega.dimx)))
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])
        Omega_prime = Omega.take(list(range(2,Omega.dimx)), list(range(2,Omega.dimx)))
        Xi_prime = Xi.take(list(range(2,Omega.dimx)), [0])
        # print(A.dimx,A.dimy, B.dimx, B.dimy)
