...
def search(grid, init, goal, cost, delta):
    ...
    # print('grid', grid, visited)
    while(q):
        cur_cost, row, col = q.popleft()
        visited[row][col] = True
        for action in range(len(delta)):
            x2 = row + delta[action][0]
            y2 = col + delta[action][1]
            if(x2<n_row and x2>=0 and y2<n_col and y2>=0 and visited[x2][y2]==False):
                if(grid[x2][y2]==goal):
                    grid[x2][y2]=0
                    # print(2*(cur_cost+cost[action]), grid)
                    return 2*(cur_cost+cost[action]), grid
                elif(grid[x2][y2]==0):
                    q.append((cur_cost+cost[action],x2,y2))
    # raise Error
    
def plan(warehouse, dropzone, todo):
    action_cost = [1, 1, 1, 1, 1.5, 1.5, 1.5, 1.5]
    delta = [[-1, 0], # go up
         [ 0,-1], # go left
         [ 1, 0], # go down
         [ 0, 1], # go right
    ...
    for i in range(len(todo)):
        # print(warehouse, dropzone, todo[i], action_cost, delta)
        cost, warehouse = search(warehouse, dropzone, todo[i], action_cost, delta)
        # print(cost, warehouse)
        total_cost+=cost
    return total_cost
