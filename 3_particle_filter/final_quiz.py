...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        s_noise = random.gauss(0, self.steering_noise)
        d_noise = random.gauss(0, self.distance_noise)
        motion[1]+=d_noise
        motion[0]+=s_noise
    ...
    # sense: 
    #    
    def sense(self, noise=None): #do not change the name of this function
        Z = []
        ...
        for i in range(len(landmarks)):
            Z.append((atan2(landmarks[i][0]-self.y, landmarks[i][1]-self.x)-self.orientation+noise)%((2*pi)))
