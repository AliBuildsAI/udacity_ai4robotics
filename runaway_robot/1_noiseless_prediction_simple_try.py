...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER is None:
        OTHER = measurement
        print(measurement, 'first time')
        return OTHER, OTHER
    elif(len(OTHER)==2):
        print(measurement, 'second time')
        d = sqrt((measurement[0]-OTHER[0])**2 + (measurement[1]-OTHER[1])**2)
        
        theta = atan2((measurement[1]-OTHER[1]),(measurement[0]-OTHER[0]))
        print(d, theta, ' d and theta')
        
        x2 = d*cos(theta)+measurement[0]
        y2 = d*sin(theta)+measurement[1]
        print(x2, y2, 'second time x2, y2')
        return [x2, y2], [measurement[0], measurement[1], d, theta%(2*pi)]

    elif(len(OTHER)==4):
        print(OTHER)
        theta = atan2((measurement[1]-OTHER[1]),(measurement[0]-OTHER[0]))
        d= OTHER[2]
        print(measurement, 'after second time')
        print(d, theta, ' d and theta')
        turning = theta - OTHER[3]
        heading = theta - turning
        new_theta = (theta+turning)%(2*pi)
        d = OTHER[2]
        x2 = d*cos(new_theta)+measurement[0]
        y2 = d*sin(new_theta)+measurement[1]
        return (x2, y2), None
